...
export const loadConnectedConvexProjectCredentials = query({
  ...
  handler: async (ctx, args) => {
    ...
    const credentials = await ctx.db
      ...
      .withIndex("bySlugs", (q) =>
        q
          .eq("teamSlug", project.teamSlug)
          .eq("projectSlug", project.projectSlug),
      )
      .first();
    if (!credentials) {
      return null;
...
export async function startProvisionConvexProjectHelper(
  ...
  if (session.memberId === undefined) {
    throw new ConvexError({
      code: "NotAuthorized",
      message: "Must be logged in to connect a project",
    });
  }
  // OAuth flow
  if (args.projectInitParams === undefined) {
    console.error(
      `Must provide projectInitParams for oauth: ${args.sessionId}`,
    );
    throw new ConvexError({
      code: "NotAuthorized",
      message: "Invalid flow for connecting a project",
    });
  }

  await ctx.scheduler.runAfter(
    0,
    internal.convexProjects.connectConvexProjectForOauth,
    {
      sessionId: args.sessionId,
      chatId: args.chatId,
  ...
  const jobId = await ctx.scheduler.runAfter(
    CHECK_CONNECTION_DEADLINE_MS,
    internal.convexProjects.checkConnection,
    {
      sessionId: args.sessionId,
      chatId: args.chatId,
    },
...
export const recordProvisionedConvexProjectCredentials = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!chat) {
      console.error(
        `Chat not found: ${args.chatId}, sessionId: ${args.sessionId}`,
      );
      return;
    }
    ...
    await _connectConvexProjectForMember(ctx, {
      ...
      .then(async (data) => {
        await ctx.runMutation(
          internal.convexProjects.recordProvisionedConvexProjectCredentials,
          {
            sessionId: args.sessionId,
            chatId: args.chatId,
      ...
      .catch(async (error) => {
        ...
        const errorMessage =
          error instanceof ConvexError
            ...
            : "Unexpected error";
        await ctx.runMutation(
          internal.convexProjects.recordFailedConvexProjectConnection,
          {
            sessionId: args.sessionId,
            chatId: args.chatId,
            errorMessage,
...
async function _connectConvexProjectForMember(
  ...
  const projectDeployKeyResponse = await fetch(
    `${bigBrainHost}/api/dashboard/authorize`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  const projectDeployKeyData: { accessToken: string } =
    await projectDeployKeyResponse.json();
  const projectDeployKey = `project:${args.teamSlug}:${data.projectSlug}|${projectDeployKeyData.accessToken}`;
  const warningMessage =
    data.projectsRemaining <= 2
      ? `You have ${data.projectsRemaining} projects remaining on this team.`
      : undefined;
...
export const checkConnection = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(chat._id, {
      convexProject: {
        kind: "failed",
        errorMessage: "Failed to connect to project",
      },
    });
  },
});
